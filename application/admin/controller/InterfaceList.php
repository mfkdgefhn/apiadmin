...
//        $listInfo = AdminList::all(['status' => 1])->order('groupHash', 'asc');
        $listInfo = AdminList::where('status', 1)->order('groupHash', 'asc')->select();
//        return $listInfo;
        foreach ($listInfo as $value) {
            $api_jk = strstr(addslashes($value->apiClass), '/', TRUE);

//            $apiRoutePath = ROOT_PATH . 'application/' . $api_jk . 'Route.php';
//            $tplPath = ROOT_PATH . 'data/' . $api_jk . 'Route.tpl';

            $tplStr .= 'Route::rule(\'' . $api_jk . '/' . addslashes($value->hash) . '\',\'' . $api_jk . '/' . addslashes($value->apiClass) . '\', \'' . $methodArr[$value->method] . '\', [\'after_behavior\' => $afterBehavior]);';
        }
